        ...
        if (!hasError)
          Expanded(
            child: InAppWebView(
              ...
              initialUrlRequest: URLRequest(url: WebUri(articleController.articleUrl), headers: WebViewSettings.getPlatformOptimizedHeaders()),
...
mixin ArticlePageBLoC on State<ArticleWebWidget> {
  ...
  void _handleWebViewError(InAppWebViewController controller, WebResourceRequest request, WebResourceError error) {
    ...
    // 检查是否是可忽略的错误类型
    final isIgnorableError = WebViewUtils.isIgnorableError(errorType, url, domain);
    ...
    if (isMainFrameRequest) {
      setState(() {
        isLoading = false;
        hasError = true;
        errorMessage = WebViewUtils.generateUserFriendlyErrorMessage(errorType, error.description ?? '', url);
      });
    }
  }
